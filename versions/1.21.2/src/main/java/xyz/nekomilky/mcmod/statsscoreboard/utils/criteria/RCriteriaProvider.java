...
public class RCriteriaProvider implements CriteriaProvider {
	...
	public List<String> getBlocks() {
		...
		for (var block : Registries.BLOCK) {
			var key = Registries.BLOCK.getId(block);
	...
	public List<String> getEntities() {
		...
		for (var entity : Registries.ENTITY_TYPE) {
			var key = Registries.ENTITY_TYPE.getId(entity);
			String id = key.getNamespace() + "." + key.getPath();
	...
	public List<String> getItems() {
		...
		for (var item : Registries.ITEM) {
			var key = Registries.ITEM.getId(item);
			String id = key.getNamespace() + "." + key.getPath();
	...
	public List<String> getCustoms() {
		List<String> list = new ArrayList<>();
		for (var custom : Registries.CUSTOM_STAT) {
	...
	public boolean isBlock(String id) {
		var identifier = id.contains(".")
			? Identifier.trySplitOn(id, '.')
			: Identifier.tryParse(id);
		return identifier != null && Registries.BLOCK.getOptionalValue(identifier).isPresent();
	}
}
